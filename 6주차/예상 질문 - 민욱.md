...
### SOP가 필요한 이유가 무엇인가?
[네이버]와 [나쁜 네이버]서버가 있다고 하자. 나쁜 네이버는 네이버 페이지를 흉내낸 해커들의 홈페이지이다. 만약 내가 [나쁜 네이버]서버에 접속했다고 하자. [나쁜 네이버]는 HTML,CSS,JS파일들을 전송하고 브라우저는 JS파일을 실행시킨다. 내 브라우저에는 네이버 로그인에 필요한 Cookie가 존재한다. 만약 SOP가 보장되지 않는다면 해커들은 해당 JS파일에 브라우저에 저장된 Cookie를 이용해 네이버에 로그인하고 정보를 본인들 서버로 전송하는 일을 할 수 있다.
