...
반대로 Connection Pool의 크기가 너무 작아지면 어떤 일이 발생할 지 생각해 보자. 가장 일반적으로 DB에 접근하려는 Thread들은 원하는 Connection을 얻지 못하고 Block되어 있는 시간이 늘어날 것이다. 이는 서버의 성능에 악영향을 끼친다. 하지만 특수한 경우에는 단순 응답 지연현상보다 더 큰 문제인 Dead Lock문제를 만날 수도 있다.
