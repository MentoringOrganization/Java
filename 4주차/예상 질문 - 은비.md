...
- Oracle Docs : https://docs.oracle.com/javase/specs/jvms/se11/html/jvms-2.html
...
````
try {
    Thread.sleep(1000); // 1초 동안 현재 스레드를 일시 중지
...
try {
    thread.join(); // 현재 스레드가 thread가 종료될 때까지 기다립니다.
} catch (InterruptedException e) {
    e.printStackTrace();
}
````
...
synchronized (obj) { // 동기화된 코드 블록
    try {
        obj.wait(); // 현재 스레드를 일시 중지하고 obj의 모니터 락을 해제합니다.
    } catch (InterruptedException e) {
        e.printStackTrace();
    }
}
...
- Oracle Docs : https://docs.oracle.com/en/java/javase/11/docs/api/java.base/java/lang/Thread.html#interrupt()
...
왜냐하면 해당 스레드의 일시 중지 상태를 종료하고, 쓰레드가 대기 중인 작업을 중단하도록 하기 위해서이다.
